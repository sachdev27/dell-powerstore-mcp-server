"""Main entry point for PowerStore MCP Server (stdio transport).
...
from .exceptions import ConfigurationError, PowerStoreMCPError
...
async def main() -> int:
    ...
    try:
        ...
        logger.info(
            "Starting Dell PowerStore MCP Server",
            extra={"transport": "stdio", "mode": "credential-free"},
        )

        # Create MCP server
        mcp_server = PowerStoreMCPServer(config)
...
def run() -> None:
    ...
    for the powerstore-mcp command.
    """
